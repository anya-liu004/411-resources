...
class BattleModel:
    ...
    def battle(self) -> str:
        """
        Starts a battle between the two combatants, determining a winner based on calculated scores
    ...
    def get_battle_score(self, combatant: Meal) -> float:
        """
        Calculates the battle score for a given meal based on its price, cuisine, and difficulty.

        Args:
            combatant (Meal): The meal whose score is to be calculated.
    ...
    def get_combatants(self) -> List[Meal]:
        """
        Retrieves the list of current combatants.
    ...
    def prep_combatant(self, combatant_data: Meal):
        """
        Adds a meal to the list of combatants if there are fewer than two.

        Args:
            combatant_data (Meal): The meal to add to combatants.
